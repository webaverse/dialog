...
class Room extends EventEmitter
{
	...
	notifyPeers(peer, keys) {
		for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
			otherPeer.notify('edit', { status: 'update', message: 'Another peer edited, refetch the keys.', keys: keys});
		} 	
	}
				...
				try {
					...
					for (let i = 0; i < keys.length; i++) {
						if (newBuffers[i]) {
							...
							await fileHandle.writeFile(newBuffers[i]);
							fileHandle.close();
						}
					}
					for (const k of keys) {
					...
					peer.notify('edit', {type: 'success', message: 'Your request to edit is successful!', keys: keys});
					this.notifyPeers(peer, keys)
				} catch (e) {
					console.error(e);
				}
			}
