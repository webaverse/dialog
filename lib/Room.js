...
const { writeFile, getFile, evalSrc, lockFiles, unlockFiles } = require('../fileUtils')
	...
	handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		peer.on('notification', async (notification) =>
		{
			...
			if (method === 'setState') {
				...
				peer.notify('deleteState', update)
				for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
					otherPeer.notify('updateState', update);
				} 
				...
				peer.notify('getState', update)
				for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
					otherPeer.notify('getState', update);
				} 	
			} else if (method === 'getAllState') {
				const update = {
					value: this.state,
				};			
				peer.notify('getAllState', update)
				for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
					otherPeer.notify('getAllState', update);
				} 	
			} else if (method === 'runScript') {
				const { key, file } = data;
				console.log(key, file)	
				await writeFile(file.name, file.src);	
				peer.notify('runScript', file.name)
				...
				const file = data.keys.file;
				const isLock = await lockFiles(keys);
				if (isLock) {
					for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
						otherPeer.notify('edit', { message: 'Chunks locked:', keys});
					} 					
					evalSrc(file.src)
					const isUnlocked = await unlockFiles(keys);
					if (isUnlocked) {
						peer.notify('edit', { message: "Edited chunks:", keys })
						for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
							otherPeer.notify('edit', { message: 'Chunks unlocked:', keys});
						} 	
					} else {
						console.error('files are not unlocking, major issue.')
					}
				} else {
					peer.notify('edit', { message: "failed to lock :(" })
				}
			} else {
