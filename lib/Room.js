...
const { writeFile, getFile } = require('../fileUtils')
	...
	handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		peer.on('notification', async (notification) =>
		{
			...
			if (method === 'setState') {
				...
				await writeFile(object.name, object.src);	
				peer.notify('runScript', object.name)
				for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
					otherPeer.notify('runScript', { key: object.name });
				} 	
