	...
	handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		peer.on('notification', async (notification) => {
			...
			const edit = async (keys, fn) => {
				...
				peer.notify('edit', {
					type: 'pending', 
					message: 'Your request to edit is received and pending.', 
					keys: keys,
					peerId: peer._id,
				});
				for (const k of keys) {
				...
				peer.notify('edit', {
					type: 'success', 
					message: 'Your request to edit is successful!', 
					keys: keys,
					peerId: peer._id
				});
			...
			if (method === 'setState') {
				...
				peer.notify('setState', update)
				for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
					otherPeer.notify('updateState', update);
				}
			} else if (method === 'getFile') {
				const { key } = data;
				const src = await getFile(key);
				peer.notify('getFile', { key: key, src: src })	
			} else if (method === 'edit') {
				const script = data.keys.file.src;
				const keys = data.keys.keys;
				this.edit(keys, script, peer);
				...
				let peerUpload = peerUploads.get(peer._id);
				if (peerUpload) {
					peerUpload.uploads.push(data.binary.binaryData);
					peerUploads.set(peer._id, peerUpload);
					if (peerUpload.uploads.length === peerUpload.maxUpload) {
						eval(peerUpload.script);
						peerUploads.delete(peer._id);
					}
				}
			} else {
			...
			case 'join':
			{
                ...
                this.loadPromise.finally(() => {
                	for (const key in this.state) {
						...
						peer.notify('updateState', update);
                	}
                	peer.notify('initState', {});
				});
