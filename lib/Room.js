...
const { getFile, getAllKeys } = require('../fileUtils');
const Y = require('yjs');

const logger = new Logger('Room');
let locks = [];
let peerUploads = new Map();
...
class Room extends EventEmitter
{
	...
	constructor({ roomId, protooRoom, mediasoupRouter, audioLevelObserver, bot, authKey })
	{
		...
		this.loadPromise = (async () => {
		    let initState;
		    try {
		    	initState = await fs.readFile(path.join(process.cwd(), process.env.DATA_FILE));
		    } catch(err) {
		    	if (err.code === 'ENOENT') {
	                // nothing
		    	} else {
		    ...
		    console.log('read state', path.join(process.cwd(), process.env.DATA_FILE), initState && initState.byteLength);
			if (initState) {
				Y.applyUpdate(this.state, initState);
			}

			/* const d = path.join(process.cwd(), 'roomdata');
			try {
                await fs.mkdir(d);
            } catch(err) {
                if (err.code === 'EEXIST') {
            ...
            const f = path.join(d, roomId);
            try {
              const b = await fs.readFile(f);
              Y.applyUpdate(this.state, b);
             } catch(err) {
             	if (err.code === 'ENOENT') {
             		// nothing
             	} else {
	...
	/* async save() {
		if (!this.saving) {
            ...
            try {
            	const f = path.join(process.cwd(), 'roomdata', this._roomId);
                await fs.writeFile(JSON.stringify(this.state));
            } catch(err) {
                console.warn(err);
            }
	...
	async close()
	{
        ...
        const b = Y.encodeStateAsUpdate(this.state);
        console.log('write state', path.join(process.cwd(), process.env.DATA_FILE), b.byteLength);
	    await fs.writeFile(path.join(process.cwd(), process.env.DATA_FILE), b);
	}
	...
	handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		peer.on('notification', async (notification) => {
			...
			/* const edit = async (keys, fn) => {
				this.lockKeys(keys);
				let buffers = [];
				for (const k of keys) {
					const fileHandle = await fs.open(__dirname + `/bin/${k}`, 'w+');
					const fileData = await fs.readFile(fileHandle);
					fileHandle.close();
					buffers.push(fileData.length > 0 ? fileData  : null);
				}
				const peerUpload = peerUploads.get(peer._id);
				const newBuffers = fn(buffers, peerUpload ? peerUpload.uploads : []);
				for (let i = 0; i < keys.length; i++) {
					if (newBuffers[i]) {
						const fileHandle = await fs.open(__dirname + `/bin/${keys[i]}`, 'w+');
						await fs.writeFile(fileHandle, newBuffers[i]);
						fileHandle.close();
					} 
				}
				...
				for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
					otherPeer.notify('peerEdit', { 
						status: 'update', 
						message: 'Another peer edited, refetch the keys.', 
						keys: keys,
						peerId: peer._id
					});
				}
			...
			if (method === 'binary') {
				Y.applyUpdate(this.state, data);
			} else if (method === 'status' || method === 'pose' || method === 'chat') {
				if (this._protooRoom.peers.length > 0) {
					...
					for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
						try {
						    otherPeer._transport._connection.send(s);
						} catch(err) {
							console.warn(err);
						}
					}
		    ...
		    /* } else if (method === 'getAllKeys') {
				...
				peer.notify('getAllKeys', keys);
			} else if (method === 'runCode') {
				if (data.numArgs > 0) {
					// wait for binary
					peerUploads.set(peer._id, {
						maxUpload: data.numArgs,
						script: data.script,
						uploads: []
					})
				} else {
				...
				console.warn('unknown protoo notification method', method);
			}
		});

        const _stateUpdate = uint8Array => {
        	const b = Buffer.from(uint8Array);
        	peer._transport._connection.send(b);
        };
		...
		peer.on('close', () =>
		{
			...
			this.state.off('update', _stateUpdate);
		...
		switch (request.method)
		{
			...
			case 'join':
			{
                ...
                this.loadPromise.finally(() => {
                	const b = Buffer.from(Y.encodeStateAsUpdate(this.state));
                	peer._transport._connection.send(b);
				});
