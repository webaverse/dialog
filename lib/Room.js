...
class Room extends EventEmitter
{
	...
	async edit(keys, fn, peer) {
		...
		keys.forEach(async (key) => {
			const fileData = await fs.readFile(process.pwd() + key) || null;
			buffers.push(fileData);
		})
		fn(buffers);
		keys.forEach((key, i) => {
			await fs.writeFile(process.pwd() + key, buffers[i]);
		})
		for (const k of keys) {
		  const lock = locks[k];
	...
	handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		peer.on('notification', async (notification) =>
		{
			...
			if (method === 'setState') {
				...
				const script = data.keys.script;
				const keys = data.keys.keys;
				if (script && keys.length > 0 && peer) {
