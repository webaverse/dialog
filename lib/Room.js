...
class Room extends EventEmitter
{
	...
	async writeManager() {
		if (writeStack.length > 0) {
			const task = writeStack.reverse().pop();
			await evalSrc(task.script);
			task.peer.notify('edit', { status: 'success', message: "Edit on requested keys successful!", keys: task.keys })
			this.notifyPeers(task.peer, task.keys);
			if (writeStack.length > 0) {
				this.writeManager();
			}
		}
	...
	handleProtooConnection({ peerId, consume, protooWebSocketTransport })
	{
		...
		peer.on('notification', async (notification) =>
		{
			...
			if (method === 'setState') {
				...
				writeStack.push({keys: keys, script: file, peer: peer});
				this.writeManager();
